...
// Variadic function to chain adapters, each paired with a free function for
...
static inline void *use_adapter(Adapter *adapter, void *instance) {
  ...
  if (adapter->next_adapter) {
    ...
    // If there is an intermediate step and a free function, free the
    // intermediate result
...
static inline void free_adapter(Adapter *adapter) {
  if (adapter->next_adapter) {
    free_adapter(
        adapter->next_adapter); // Recursively free the rest of the chain
  }
